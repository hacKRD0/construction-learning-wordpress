 ...
 $billing_first_name =get_user_meta( $current_user->ID, $obj->billing_first_name_meta, true );
 $billing_last_name = get_user_meta( $current_user->ID, $obj->billing_last_name_meta,true );
 $billing_company = get_user_meta( $current_user->ID, $obj->billing_company_meta,true );
 $billing_country = get_user_meta( $current_user->ID, $obj->billing_country_meta, true);
 $billing_address_1 = get_user_meta( $current_user->ID, $obj->billing_address_1_meta, true );
 $billing_address_2 =  get_user_meta( $current_user->ID, $obj->billing_address_2_meta, true );
 $billing_city =   get_user_meta( $current_user->ID, $obj->billing_city_meta, true );
 $billing_postcode = get_user_meta( $current_user->ID, $obj->billing_postcode_meta, true );
 $billing_phone = get_user_meta( $current_user->ID, $obj->billing_phone_meta, true );
 $billing_email = get_user_meta( $current_user->ID, $obj->billing_email_meta, true );
 $billing_state = get_user_meta( $current_user->ID, $obj->billing_state_meta, true );
  ...
  <div id="tab1" class="tab-pane fade in active show">
    <div class="table-wrapper">
     <div class="table-title">
                <div class="row">
                    <div class="col-md-12"><h5 class="cs-label-1">Order Details</h5></div>
            ...
            <table class="table table-bordered">
                <thead>
                 ...
                 <tbody>
                 	...
                 	?>
                 	<tr> 
                    <td><a target="_blank" href="<?= home_url() ?>/my-account/view-order/<?= $orders->ID?>/"><?= $orders->ID; ?></a></td>
                     <td><?= $order_data['date_created']->date('M-d-Y'); ?></td>
                      <td><?= $order->get_status(); ?></td>
                       <td><?= '₹'.$order_data['total']; ?></td>
                        <td><a target="_blank" href="<?= home_url() ?>/my-account/view-order/<?= $orders->ID?>/" class="woocommerce-button wp-element-button button view">View</a></td>
                 	</tr>
                 	<?php } ?>
                 </tbody>
				...
				<form class="form"  id="updateBillingAddress" name="updateBillingAddress" method="POST">
						<div class="row">
    						<div class="col-sm-6 form-group">
    							...
    							<input name="billing_first_name" type="text"  class="form-control required" value="<?=  $billing_first_name; ?>">
    						</div>
    						<div class="col-sm-6 form-group">
    							<label>Last Name *</label>
    							<input name="billing_last_name" type="text" class="form-control required" value="<?=  $billing_last_name; ?>">
    						</div>
	                    </div>
						<div class="row">						    	
							<div class="col-sm-6 form-group">
								<label>Company name (optional)</label>
								<input name="billing_company" type="text" class="form-control required" value="<?=  $billing_company; ?>">
							</div>
						...
						<div class="row">
						    <div class="col-sm-4 form-group">
								...
								<input type="text" placeholder="House number and street name" class="form-control required"  name="billing_address_1"  value="<?= $billing_address_1; ?>">
							</div>	
							<div class="col-sm-4 form-group">
								<label>Street address *</label>
								<input type="text" placeholder="Apartment, suite, unit, etc. (optional)" class="form-control required"  name="billing_address_2"  value="<?= $billing_address_2; ?>">
							</div>	
							<div class="col-sm-4 form-group">
								<label>State *</label>
						...
						<div class="row">
						    <div class="col-sm-6 form-group">
								...
								<input type="text" name="billing_city" class="form-control required " value="<?=  $billing_city; ?>">
							</div>	
							<div class="col-sm-6 form-group">
								<label>PIN Code *</label>
								<input name="billing_postcode" type="text" class="form-control required " value="<?=  $billing_postcode; ?>">
							</div>						    
					...
					<div class="row">
						    <div class="col-sm-6 form-group">
								...
								<input type="text" name="billing_email" class="form-control required email" value="<?=  $billing_email; ?>">
							</div>	
							<div class="col-sm-6 form-group">
								<label>Phone *</label>
								<input name="billing_phone" type="text"  class="form-control required " value="<?=  $billing_phone; ?>">
							</div>	
						</div>	
       ...
       var pass1 = document.getElementById('password');
        ...
        if (jQuery('.change-password-form .password1').val() === '') {
            ...
            jQuery.post(ajaxurl, data, function(response) {
                ...
                if(response === 'success') {
                    jQuery(".change-password-messages").html('<p class = "bg-success">Password Successfully Changed <br /><a href = "<?= home_url(); ?>">Click here to continue</a></p>');
